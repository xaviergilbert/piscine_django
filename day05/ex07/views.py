...
def create_table():
    sortie = os.popen('python manage.py makemigrations ex07', "r").read()
    log = open("database.log", "a")
    log.write("\n" + sortie)
    log.close()
    try:
        os.system('python manage.py migrate ex07')
        if sortie == "No changes detected in app 'ex07'\n":
            return sortie
        else:
            return "Ok."
...
def init(request):
    ...
    return render(request, 'ex07/init.html', {'response': response})
...
def populate(request):
    ...
    return render(request, 'ex07/populate.html', {'responses': response})


def display(request):
    all_entries = movies.objects.all()
    return render(request, 'ex07/display.html', {'films': all_entries})
