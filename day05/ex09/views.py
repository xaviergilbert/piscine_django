...
def create_table():
    sortie = os.popen('python manage.py makemigrations ex09', "r").read()
    log = open("database.log", "a")
    log.write("\n" + sortie)
    log.close()
    try:
        os.system('python manage.py migrate ex09')
        if sortie == "No changes detected in app 'ex09'\n":
            return sortie
        else:
            return "Ok."
...
def init(request):
    ...
    return render(request, 'ex09/init.html', {'response': response})
...
def populate(request):
    ...
    return render(request, 'ex09/populate.html', {'responses': response})


def display(request):
    people = People.objects.all().order_by('name')
    people = people.filter(homeworld__climate__contains='windy')
    return render(request, 'ex09/display.html', {'people': people})

def update(request):
    form = UpdateForm(request.POST)
    if request.method == "POST":
        movie = movies.objects.get(episode_nb=request.POST.get('title'))       
        opening_crawl = request.POST.get('opening_crawl')
        movie.opening_crawl = opening_crawl
        movie.save()

        return redirect('ex09:update')

    else:
        form = UpdateForm()

        return render(request, 'ex09/update.html', {'form': form})
