...
def create_table():
    sortie = os.popen('python manage.py makemigrations ex05', "r").read()
    log = open("database.log", "a")
    log.write("\n" + sortie)
    log.close()
    try:
        os.system('python manage.py migrate ex05')
        if sortie == "No changes detected in app 'ex05'\n":
            return sortie
        else:
            return "Ok."
...
def init(request):
    ...
    return render(request, 'ex05/init.html', {'response': response})
...
def populate(request):
    ...
    return render(request, 'ex05/populate.html', {'responses': response})


def display(request):
    all_entries = movies.objects.all()
    return render(request, 'ex05/display.html', {'films': all_entries})


def remove(request):
    form = RemoveForm(request.POST)
    if request.method == "POST":
        item = movies.objects.get(episode_nb=request.POST.get('choice'))       
        item.delete()
